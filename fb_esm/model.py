...
from .embedding import BERTEmbeddingConvESM
...
class ProteinBertModel(nn.Module):
    ...
    def _init_submodules_common(self):
        if self.emb_type == 'conv':
                self.embed_tokens = BERTEmbeddingConvESM(
    ...
    def forward(self, tokens, repr_layers=[], need_head_weights=False, return_contacts=False):
        ...
        if self.emb_type == 'conv':
            emb_tokens,_ = self.embed_tokens(tokens)
        else:
            emb_tokens = self.embed_tokens(tokens)

        # stop()
        # print(emb_tokens.shape)
        x = self.embed_scale * emb_tokens
